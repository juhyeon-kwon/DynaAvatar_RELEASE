...
from LHM.runners.infer.utils import (
    ...
    prepare_motion_seqs,
    prepare_motion_seqs_2,
    prepare_motion_seqs_from_npz,
...
def parse_configs():
    ...
    motion_frame_idx_list = [motion_seq_path.split('/')[-1][:-5] for motion_seq_path in glob(os.path.join(cfg.motion_seqs_dir, '*.json'))]
    motion_frame_idx_list = sorted([int(x) for x in motion_frame_idx_list])
        
    if "start_frame_idx" in cli_cfg and str(cli_cfg.start_frame_idx) != "None":
        try:
            cfg.motion_st_idx = motion_frame_idx_list.index(int(cli_cfg.start_frame_idx))
        except: 
            print(f"[VCAI] {int(cli_cfg.start_frame_idx)} not in motion_seqs_dir! Set first frame idx as motion_st_idx")
            cfg.motion_st_idx = 0
    else:
        cfg.motion_st_idx = 0
    
    cfg.camera_name = None
    if "cam_idx" in cli_cfg and str(cli_cfg.cam_idx) != "None":
        cfg.camera_name = str(cli_cfg.cam_idx)
        
    if "motion_size" in cli_cfg and str(cli_cfg.motion_size) != "None":
        cfg.motion_size = int(cli_cfg.motion_size)
    else:
        cfg.motion_size = len(motion_frame_idx_list)
    cfg.sampling_stride = min(cli_cfg.input_fps // 15, 1)
...
class HumanLRMInferrer(Inferrer):
    ...
    def create_motion_history(self, smplx_params, n_history_length, motion_seqs_dir, sampling_stride=1):
        ...
        transl = smplx_params['trans'].squeeze(0)  # shape: (F, 3)
        ...
        if '4D' in motion_seqs_dir or 'actorshq' in motion_seqs_dir: 
            ...
            root_pose_orig = root_pose.squeeze(2).view(-1,3)  # (F * n_history_length + 1, 3)
            root_pose_orig = axis_angle_to_matrix(root_pose_orig)  # (F * n_history_length + 1, 3, 3)
            B = root_pose_orig.shape[0]
            root_pose = matrix_to_axis_angle(torch.bmm(rot_4d_to_dna.expand(B,-1,-1), root_pose_orig)).view(smplx_params['root_pose'].shape[0], smplx_params['root_pose'].shape[1], 1, 3)
            transl = torch.bmm(rot_4d_to_dna.expand(B,-1,-1), transl[:,:,None]).view(-1,3)
            ...
            root_pose_orig = axis_angle_to_matrix(root_pose_orig)  # (F * n_history_length + 1, 3, 3)
            B = root_pose_orig.shape[0]
            root_pose = matrix_to_axis_angle(torch.bmm(rot_i3d_to_dna.expand(B,-1,-1), root_pose_orig)).view(smplx_params['root_pose'].shape[0], smplx_params['root_pose'].shape[1], 1, 3)
            transl = torch.bmm(rot_i3d_to_dna.expand(B,-1,-1), transl[:,:,None]).view(-1,3)
        
        full_pose = torch.cat([
            root_pose, body_pose, lhand_pose, rhand_pose, jaw_pose, leye_pose, reye_pose
        ], dim=2).squeeze(0)  # shape: (F, 55, 3)

        
        transl = transl.unsqueeze(1)
        full_pose_with_transl = torch.cat([full_pose, transl], dim=1) # shape: (F, 56, 3)

        F, num_features, pose_dim = full_pose_with_transl.shape # num_features는 56
        ...
        motion_history = motion_history.flip(1)

        #print("motion history shape:", motion_history.shape)
    ...
    def infer_single(
        ...
        vis_head_img = (src_head_rgb.squeeze(0).permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8)
        ...
        if motion_name in self.motion_dict:
            ...
            if self.cfg.camera_name is not None: 
                ...
                if camera_name not in camera_name_set: 
                    assert ValueError, f"Cam {camera_name} is not exists! Please check " + osp.join(motion_seqs_dir, '..', '..', 'cam_params.json')
            else:
                ...
                elif 'actorshq' in motion_seqs_dir: 
                    camera_name = '127'
                else:   # Custom
                    camera_name = '00'
    ...
    def infer(self):
        ...
        for image_path in tqdm(image_paths, disable = not self.accelerator.is_local_main_process,):
            ...
            # setting config
            motion_seqs_dir = self.cfg.motion_seqs_dir  ## smplx/smplx_params_smooth
            ...
            print("[VCAI] Predict shape from a given image!")   
            shape_pose = self.pose_estimator(image_path)
            beta = shape_pose.beta
