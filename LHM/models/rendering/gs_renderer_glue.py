...
class GS3DRenderer(nn.Module):
    ...
    def project_points(self, xyz, c2w, intrinsic, height, width):
        """
        xyz: (N,3)
        c2w: (1,4,4) camera-to-world
        intrinsic: (1,3,3) OpenCV style
        """
        c2w = c2w.squeeze(0)
        w2c = torch.inverse(c2w)
        ...
        xyz_cam = (xyz_h @ w2c.T)[:, :3]               # (N,3), (X,Y,Z)

        # (2) Intrinsic 적용 (OpenCV convention)
        intrinsic = intrinsic.squeeze(0)               # (3,3)
        fx, fy = intrinsic[0,0], intrinsic[1,1]
        cx, cy = intrinsic[0,2], intrinsic[1,2]

        X, Y, Z = xyz_cam[:,0], xyz_cam[:,1], xyz_cam[:,2] + 1e-6
        u = fx * (X / Z) + cx
        v = fy * (Y / Z) + cy
    ...
    def forward_animate_gs(
        ...
        for b in range(batch_size):
            ...
            for v in range(N_view):
                ...
                proj_coord, _ = self.project_points(hand_detached_xyz, c2w_view, intrinsic_view, height, width) # shape: (N, 2)
                
                proj_coord_norm = proj_coord.clone()
                proj_coord_norm[:,0] = proj_coord[:,0] / width   # u → [0,1]
                proj_coord_norm[:,1] = proj_coord[:,1] / height  # v → [0,1]
                zeros = torch.zeros(proj_coord_norm.shape[0], 1, device=proj_coord_norm.device)  # (N,1)
                proj_coord_norm = torch.cat([proj_coord_norm, zeros], dim=1) # shape: (N, 3)
                ...
                proj_out_list.append(
                    self.forward_single_batch(
                        [
                            GaussianModel(
                                xyz=hand_detached_xyz,
                                opacity=hand_detached_opacity,
                                rotation=hand_detached_rotation,
                                scaling=hand_detached_scaling,
                                shs=proj_coord_norm, # <-- 
                                use_rgb=self.gs_net.use_rgb,
                            )
                        ], 
