...
def getProjectionMatrix(znear, zfar, fovX, fovY):
    ...
    P[3, 2] = z_sign
    #P[2, 2] = z_sign * zfar / (zfar - znear) # original 3DGS
    #P[2, 3] = -(zfar * znear) / (zfar - znear) # original 3DGS
    P[2, 2] = z_sign * (znear + zfar) / (zfar - znear)
    P[2, 3] = -(2 * zfar * znear) / (zfar - znear)
    return P

def intrinsic_to_projection_matrix(znear, zfar, width, height, intrinsic):
    '''tanHalfFovY = math.tan((fovY / 2))
    tanHalfFovX = math.tan((fovX / 2))
    top = tanHalfFovY * znear
    bottom = -top
    ...
    # near 평면에서 비대칭 frustum (카메라 좌표계, y는 위가 + 라는 전제)
    left = -cx * znear / fx
    right = (width - cx) * znear / fx
    bottom = -cy * znear / fy
    top = (height - cy) * znear / fy
    P = torch.zeros(4, 4)
    z_sign = 1.0
    P[0, 0] = 2.0 * znear / (right - left)
    P[1, 1] = 2.0 * znear / (top - bottom)
...
class Camera:
    def __init__(
        ...
        self.projection_matrix = (
            intrinsic_to_projection_matrix(
                self.znear, self.zfar, self.width, self.height, intrinsic
            )
            .transpose(0, 1)
            .to(w2c.device)
        )
...
class GSLayer(nn.Module):
    ...
    def __init__(
        ...
        self.use_skinning_offset = use_skinning_offset
        ...
        # qw00n;
        if self.use_skinning_offset:
            self.out_layers["offset_skinning_weight"] = nn.Linear(in_channels, 55)
    ...
    def forward(self, x, pts, x_fine=None, constrain_dict=None):
        ...
        for k in self.attr_dict:
            ...
            if k == "rotation":
                ...
                if self.clip_scaling is not None: # qw00n; clip_scaling 0.05
        ...
        # qw00n;
        output = GaussianAppOutput(**ret)
        if self.use_skinning_offset:
            output.offset_skinning_weight = self.out_layers['offset_skinning_weight'](x)
        else:
            output.offset_skinning_weight = None
        return output
...
class PointEmbed(nn.Module):
    def __init__(self, hidden_dim=48, dim=128):
        ...
        e = torch.pow(2, torch.arange(self.embedding_dim // 6)).float() * np.pi # qw00n; [8]
    ...
    def embed(input, basis):
        ...
        return embeddings # B, N, 48

    def forward(self, input):
        # input: B x N x 3
        embed = self.mlp(
            torch.cat([self.embed(input, self.basis), input], dim=2) # qw00n; concat original xyz with positional embedding
        )  # B x N x C
...
class GS3DRenderer(nn.Module):
    def __init__(
        ...
        dense_sample_pts=80000,  # only use for dense_smaple_smplx
        restrict_offset=True,
        ...
        self.gs_net = GSLayer(
            ...
            init_density=0.1,
            xyz_offset=True,
            restrict_offset=restrict_offset, # qw00n;
            ...
            use_skinning_offset=use_skinning_offset
        )
    ...
    def animate_gs_model(
        ...
        with torch.autocast(device_type=device.type, dtype=torch.float32):
            ...
            if gs_attr.offset_skinning_weight is not None:
                ...
                offset_skinning_weight = gs_attr.offset_skinning_weight # None
            mean_3d, transform_matrix = (
                self.smplx_model.transform_to_posed_verts_from_neutral_pose(
                    ...
                    use_skinning_offset = self.use_skinning_offset,
    ...
    def forward_single_batch(
        ...
        for v_idx, (c2w, intrinsic) in enumerate(zip(c2ws, intrinsics)):
            # qw00n: to match the convention
            intrinsic_for_3dgs = intrinsic.clone()
            intrinsic_for_3dgs[1, 2] = height - intrinsic[1, 2]
            intrinsic_for_3dgs[0, 2] = width - intrinsic[0, 2]

            out_list.append(
                self.forward_single_view(
                    gs_list[0], # qw00n; was v_idx
                    Camera.from_c2w(c2w, intrinsic_for_3dgs, height, width),
                    background_color[0], # qw00n; was v_idx
                )
            )
    ...
    def forward_single_batch_eval(
        ...
        target_dir = os.path.join(save_path[0], uid[0])
        all_items = os.listdir(target_dir)
        folder_names = [name for name in all_items 
                        if os.path.isdir(os.path.join(target_dir, name))]
        sorted_cam_folder_list = sorted(folder_names, key=int)

        for v_idx, (c2w, intrinsic) in enumerate(zip(c2ws, intrinsics)):
            # qw00n: to match the convention
            intrinsic_for_3dgs = intrinsic.clone()
            
            intrinsic_for_3dgs[1, 2] = height[0][v_idx] - intrinsic[1, 2]
            intrinsic_for_3dgs[0, 2] = width[0][v_idx] - intrinsic[0, 2]

            eval_out = self.forward_single_view(
                    gs_list[0], # qw00n; was v_idx
                    Camera.from_c2w(c2w, intrinsic_for_3dgs, height[0][v_idx].item(), width[0][v_idx].item()),
                    background_color[0], # qw00n; was v_idx
            )

            cam_name = sorted_cam_folder_list[v_idx]
            dataset = save_path[0].split('/')[-2]

            file_name = f'{frame_indice:06d}.png'
            img_path = os.path.join(save_path[0], uid[0], cam_name, file_name)
            
            #img_path = os.path.join(save_path[0], uid[0], cam_name, file_name)
            
            cv2.imwrite(
                img_path,
                (eval_out["comp_rgb"].detach().cpu().numpy()[..., ::-1] * 255).astype(
                    np.uint8
                ),
            )
    ...
    def forward_gs(
        ...
        **kwargs,
        ...
        # qw00n; skip decoder: use transformed latent gs_hidden_features as query_gs_features
    ...
    def project_points(self, xyz, c2w, intrinsic, height, width):
        ...
        u = ((ndc_x + 1.0) * width - 1.0) * 0.5
        v = ((ndc_y + 1.0) * height - 1.0) * 0.5
    ...
    def forward_animate_gs(
        ...
        # qw00n; canon rendering
        '''
        smplx_data["root_pose"] = torch.FloatTensor([[[-3.0881, -0.0213, -0.5706]]]).cuda()
        smplx_data["body_pose"] = torch.zeros_like(smplx_data["body_pose"])
        smplx_data["body_pose"][0][0][0] = torch.FloatTensor([0, 0, math.pi / 16])
        smplx_data["body_pose"][0][0][1] = torch.FloatTensor([0, 0, -math.pi / 16])
        smplx_data["lhand_pose"] = torch.zeros_like(smplx_data["lhand_pose"])
        smplx_data["rhand_pose"] = torch.zeros_like(smplx_data["rhand_pose"])
        smplx_data["trans"] = torch.FloatTensor([[[-0.0163,  0.3015, -0.1817]]]).cuda()
        '''
        
        #print('#@#!! 1', batch_size)
        #print('#@#!! 3', c2w.shape)
        #print('#@#!! 4', intrinsic.shape)
        #print('#@#!! 5', smplx_data['body_pose'].shape)
        #print('#@#!! 6', query_points.shape)
        ...
        for b in range(batch_size):
            ...
            merge_animatable_gs_model_list, cano_gs_model_list, mask_gs_list = self.animate_gs_model(
                gs_attr,
                query_pt,
            ...
            mask_gs_list = mask_gs_list[:N_Ref]

            #print(animatable_gs_model_list[0].opacity.shape) # N, 1
            #print(animatable_gs_model_list[0].scaling.shape) # N, 3
            ...
            # qw00n; for mask distribution loss
            mask_out_list.append(
                self.forward_single_batch(
                    mask_gs_list,
                    c2w[b],
                    intrinsic[b],
            ...
            # qw00n; for glue loss
            hand_mask = (
                    ((self.smplx_model.is_rhand + self.smplx_model.is_lhand) > 0)
            ) 

            hand_detached_xyz = torch.where(hand_mask.unsqueeze(1), animatable_gs_model_list[0].xyz.detach(), animatable_gs_model_list[0].xyz)
            hand_detached_opacity = torch.where(hand_mask.unsqueeze(1), animatable_gs_model_list[0].opacity.detach(), animatable_gs_model_list[0].opacity)
            hand_detached_rotation = torch.where(hand_mask.unsqueeze(1), animatable_gs_model_list[0].rotation.detach(), animatable_gs_model_list[0].rotation)
            hand_detached_scaling = torch.where(hand_mask.unsqueeze(1), animatable_gs_model_list[0].scaling.detach(), animatable_gs_model_list[0].scaling)
            
            # view-dependent rendering
            for v in range(N_view):
                c2w_view = c2w[b, v:v+1] # shape: (1, 4, 4)
                intrinsic_view = intrinsic[b, v:v+1, :3, :3] # shape: (1, 3, 3)

                # project_points는 [-0.5, S-0.5] 범위의 좌표 반환
                proj_coord, _ = self.project_points(hand_detached_xyz, c2w_view, intrinsic_view, height, width) 
                                
                proj_coord_norm = proj_coord.clone()

                # align_corners=False 규약에 맞게 [0, 1]로 정규화
                # [-0.5, W-0.5] -> [0, 1] ( (v + 0.5) / W )
                proj_coord_norm[:,0] = (proj_coord[:,0] + 0.5) / width
                # [-0.5, H-0.5] -> [0, 1] ( (v + 0.5) / H )
                proj_coord_norm[:,1] = (proj_coord[:,1] + 0.5) / height

                proj_coord_norm = torch.clamp(proj_coord_norm, 0.0, 1.0)

                zeros = torch.zeros(proj_coord_norm.shape[0], 1, device=proj_coord_norm.device)
                proj_coord_norm = torch.cat([proj_coord_norm, zeros], dim=1)

        

                proj_out_list.append(
                    self.forward_single_batch(
                        [
                            GaussianModel(
                                xyz=animatable_gs_model_list[0].xyz.detach(),
                                opacity=torch.ones_like(animatable_gs_model_list[0].opacity.detach()),
                                rotation=animatable_gs_model_list[0].rotation.detach(),
                                scaling=animatable_gs_model_list[0].scaling.detach() * 1,
                                shs=proj_coord_norm, # <-- 
                                use_rgb=self.gs_net.use_rgb,
                            )
                        ], 
                        c2w[b, v:v+1],                 # shape: (N_view, 4, 4)
                        intrinsic[b, v:v+1],           # shape: (N_view, 4, 4)
                        height,
                        width,
                        torch.zeros_like(background_color[b]),
                        debug=debug,
                    )
                )
        ...
        out = defaultdict(list)
        mask_out = defaultdict(list)
        proj_out = defaultdict(list)
        for out_ in out_list:
            for k, v in out_.items():
                out[k].append(v)
        ...
        for out_ in mask_out_list:
            for k, v in out_.items():
                mask_out[k].append(v)
        for k, v in mask_out.items():
            if isinstance(v[0], torch.Tensor):
                mask_out[k] = torch.stack(v, dim=0)
            else:
                mask_out[k] = v
        
        for out_ in proj_out_list:
            for k, v in out_.items():
                proj_out[k].append(v)
        for k, v in proj_out.items():
            if isinstance(v[0], torch.Tensor):
                proj_out[k] = torch.stack(v, dim=0)
            else:
                proj_out[k] = v
        ...
        out["comp_rgb"] = out["comp_rgb"].permute(
            0, 1, 4, 2, 3
        ).contiguous()  # [B, NV, H, W, 3] -> [B, NV, 3, H, W]
        out["comp_mask"] = out["comp_mask"].permute(
            0, 1, 4, 2, 3
        ).contiguous()  # [B, NV, H, W, 3] -> [B, NV, 1, H, W]
        out["comp_depth"] = out["comp_depth"].permute(
            0, 1, 4, 2, 3
        ).contiguous()  # [B, NV, H, W, 3] -> [B, NV, 1, H, W]

        mask_out["comp_mask"] = mask_out["comp_mask"].permute(
            0, 1, 4, 2, 3
        ).contiguous()
        out["dist_comp_mask"] = mask_out["comp_mask"]
        
        proj_out["comp_rgb"] = proj_out["comp_rgb"].permute(
            0, 1, 4, 2, 3
        ).contiguous()
        out["proj_comp_rgb"] = proj_out["comp_rgb"]

        proj_out["comp_mask"] = proj_out["comp_mask"].permute(
            0, 1, 4, 2, 3
        ).contiguous()
        out["proj_comp_mask"] = proj_out["comp_mask"]
        

        return out

    def forward_animate_gs_eval(
        self,
        gs_attr_list,
        query_points,
        smplx_data,
        ...
        for b in range(batch_size):
            ...
            # gs render animated gs model.
            self.forward_single_batch_eval(
                animatable_gs_model_list,
                c2w[b],
                intrinsic[b],
                height[b], ##
