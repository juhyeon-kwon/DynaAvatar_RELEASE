...
class ModelHumanLRMSapdinoBodyHeadSD3_5(ModelHumanLRM):
    ...
    def animation_infer_face_blend(self, gs_model_list, query_points, smplx_params, render_c2ws, render_intrs, render_bg_colors):
        """Like animation_infer(is_dynamic=True) but uses face-blend skinning:
        face/hand vertices use skinning_weight_original (LHM), body uses offset_skinning_weight (DynaAvatar)."""
        ...
        for view_idx in range(num_views):
            render_res = self.renderer.forward_animate_gs_face_blend(
                [gs_model_list[view_idx]],
                query_points[view_idx].unsqueeze(0),
                self.renderer.get_single_view_smpl_data(smplx_params, view_idx),
                render_c2ws[:, view_idx : view_idx + 1],
