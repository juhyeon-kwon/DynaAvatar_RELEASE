...
class SD3MMJointTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        # qw00n; params report
        '''
        print("\n--- [SD3MMJointTransformerBlock] Parameter calc ---")
        module_names = ['norm1', 'norm1_context', 'attn', 'norm2', 'ff', 'norm2_context', 'ff_context']
        for name in module_names:
            if hasattr(self, name) and getattr(self, name) is not None:
                module = getattr(self, name)
                params = sum(p.numel() for p in module.parameters())
                print(f"  - Module '{name}': {params:,} 개")
        total_params = sum(p.numel() for p in self.parameters())
        print("-" * 50)
        print(f"  ▶ Total params: {total_params:,} 개")
        print("=" * 50 + "\n")
        '''
...
# qw00n; modified for dynamic block
class SD3BodyHeadMMJointTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        # qw00n;
        if self.is_dynamic:
            self.dynamic_dit = SD3MMJointTransformerBlock(
                dim,
                num_heads,
                eps,
                context_pre_only=context_pre_only,
    ...
    def forward(
        ...
        # qw00n; split the input tokens into head / body
        ...
        # qw00n;
        if self.is_dynamic:
            hidden_states, dynamic_encoder_hidden_states  = self.dynamic_dit(
                hidden_states, encoder_hidden_states2, temb2
            )
            return hidden_states, encoder_hidden_states, dynamic_encoder_hidden_states
        else:
            return hidden_states, encoder_hidden_states
    
# qw00n; implemented (old ver), reused as post layer
class SD3DynamicMMJointTransformerBlock(nn.Module):
    r"""
    ...
    def forward(
        ...
        # qw00n; dynamic dit block
        hidden_states, dynamic_encoder_hidden_states  = self.dynamic_dit(
            hidden_states, encoder_hidden_states2, temb2
        )


        return hidden_states, dynamic_encoder_hidden_states
