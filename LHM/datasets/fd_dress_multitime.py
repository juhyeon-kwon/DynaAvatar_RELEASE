...
class FDDressDataset(BaseDataset):
    ...
    def inner_get_item(self, idx):
        ...
        #print("uid:", uid, "ref_cam_idx:", ref_cam_idx, "min_frame_idx:", min_frame_idx, "max_frame_idx:", max_frame_idx)
        ...
        start_sampling_idx = int(min_frame_idx) + sampling_stride * (self.n_history_length+self.num_train_frames-1)  # jane; TODO: fix hard-coding
        end_frame_idx_int = random.randint(min(start_sampling_idx, int(max_frame_idx)), int(max_frame_idx))  # 1004
        ...
        '''for i in range(self.num_train_frames): 
            current_frame_idx_int = end_frame_idx_int - (self.num_train_frames-1 + i) * sampling_stride
            current_frame_idx_str = f'{current_frame_idx_int:06d}'
            cv2.imwrite(os.path.join(f'{uid}_{tar_cam_idx}_{current_frame_idx_str}_img.jpg'), gt_imgs_tensor[i,:,:,:].clone().detach().cpu().numpy().transpose(1,2,0)*255.)
            cv2.imwrite(os.path.join(f'{uid}_{tar_cam_idx}_{current_frame_idx_str}_mask.jpg'), gt_masks_tensor[i,:,:,:].clone().detach().cpu().numpy().transpose(1,2,0)*255.)'''
