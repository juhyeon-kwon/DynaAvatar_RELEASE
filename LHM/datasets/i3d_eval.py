...
class I3DDataset(BaseDataset):
    ...
    def __init__(self, root_dirs: list[str], meta_path: str,
        ...
        for uid in tqdm(self.uids):
            try:
                ...
                if not cam_params:
                    print(f"[{uid}] Warning: No cam_params.json, skipping.")
                    continue
                
                if 'ID1_1' in uid: 
                    eval_cam_indices = [2, 4, 7, 9]
                elif 'ID1_2' in uid: 
                    eval_cam_indices = [1, 6, 8, 9]
                elif 'ID2_1' in uid: 
                    eval_cam_indices = [2, 4, 6, 10]
                else:  # id2_2, id3_1, id3_2
                    eval_cam_indices = [2, 4, 6, 8, 10]
                ...
                for cam_idx_int in eval_cam_indices:
                    cam_name = f'{cam_idx_int:02d}'
                    if cam_name not in cam_params:
                        continue
                    cam_param = {k: np.array(v) for k, v in cam_params[cam_name].items()}
                    ...
                    if len(frames_for_cam) == 0:
                        continue
                    frames_for_cam.sort(key=lambda x: int(x))
                    sample_frame_idx = frames_for_cam[0]
                    ex_img_b = toc.read_image_bytes(uid, cam_name, sample_frame_idx)
                    ex_image = np.array(Image.open(io.BytesIO(ex_img_b)).convert('RGB'))
                    height, width = ex_image.shape[:2]

                    all_intri_list.append(K_4x4)
                    all_extri_list.append(c2w_4x4)
                ...
                min_frame_int = int(min_frame)
                max_frame_int = int(max_frame)

                if uid in ('ID1_1-novelpose'): 
                    first_valid_end_frame = min_frame_int + (self.num_train_frames - 1) * 10
                elif uid in ('ID1_1-novelview', 'ID1_2-novelpose', 'ID1_2-novelview'): 
                    first_valid_end_frame = min_frame_int + (self.num_train_frames - 1) * 50
                elif uid in ('ID2_1-novelpose', 'ID3_1-novelpose'): 
                    first_valid_end_frame = min_frame_int + (self.num_train_frames - 1) * 15
                elif uid in ('ID2_1-novelview', 'ID2_2-novelpose', 'ID2_2-novelview', 'ID3_2-novelpose', 'ID3_2-novelview'): 
                    first_valid_end_frame = min_frame_int + (self.num_train_frames - 1) * 30
                elif uid in ('ID3_1-novelview'): 
                    first_valid_end_frame = min_frame_int + (self.num_train_frames - 1) * 60
                else: 
                ...
                if uid in ('ID1_1-novelpose'): 
                    ...
                    eval_chunk_skip = self.num_train_frames * 50
                elif uid in ('ID2_1-novelpose', 'ID3_1-novelpose'): 
                    eval_chunk_skip = self.num_train_frames * 15
                elif uid in ('ID2_1-novelview', 'ID2_2-novelpose', 'ID2_2-novelview', 'ID3_2-novelpose', 'ID3_2-novelview'): 
                    eval_chunk_skip = self.num_train_frames * 30
                else: # uid in ('ID3_1-novelview'): 
                    eval_chunk_skip = self.num_train_frames * 60
    ...
    def load_smplx_history(self, uid: str, frame_idx: str, sampling_stride: int, min_frame_idx: int) -> np.ndarray: 
        ...
        for n in range(0, self.n_history_length+1):
            ...
            if smplx_data is None: 
                ...
                rot_i3d_to_dna = torch.tensor([[1.,0.,0.],
                                    [0.,0.,-1.],
                                    [0.,1.,0.]], dtype=torch.float32)
                ## fullpose 
                pose_data = smplx_data['fullpose'].copy()
                root_pose_orig = axis_angle_to_matrix(torch.tensor(pose_data[0,:]))
                pose_data[0] = np.array(matrix_to_axis_angle(rot_i3d_to_dna @ root_pose_orig))
                trans = np.array(rot_i3d_to_dna @ torch.tensor(smplx_data['transl'])[:,None]).reshape(3)
    ...
    def inner_get_item(self, idx):
        ...
        for i in range(self.num_train_frames): 
            ...
            if uid in ('ID1_1-novelpose'): 
                current_frame_idx_int = end_frame_idx_int - (self.num_train_frames - 1 - i) * 10
            elif uid in ('ID1_1-novelview', 'ID1_2-novelpose', 'ID1_2-novelview'): 
                current_frame_idx_int = end_frame_idx_int - (self.num_train_frames - 1 - i) * 50
            elif uid in ('ID2_1-novelpose', 'ID3_1-novelpose'): 
                current_frame_idx_int = end_frame_idx_int - (self.num_train_frames - 1 - i) * 15
            elif uid in ('ID2_1-novelview', 'ID2_2-novelpose', 'ID2_2-novelview', 'ID3_2-novelpose', 'ID3_2-novelview'): 
                current_frame_idx_int = end_frame_idx_int - (self.num_train_frames - 1 - i) * 30
            else: #('ID3_1-novelview')
                current_frame_idx_int = end_frame_idx_int - (self.num_train_frames - 1 - i) * 60
            ...
            if smplx_data is None:
                raise RuntimeError(f"[{uid}] smplx_data is None for frame {current_frame_idx_str}, end_frame: {end_frame_idx_int}, i: {i}")

            num_betas = 10
            betas = np.squeeze(smplx_data['betas'])[:num_betas]
            expr = np.squeeze(smplx_data['expr'])[:num_betas]

            target_betas_list.append(torch.from_numpy(betas))
            target_exprs_list.append(torch.from_numpy(expr))
            target_pose_list.append(torch.from_numpy(smplx_data['fullpose']))
            target_transl_list.append(torch.from_numpy(smplx_data['transl']))

            skeleton_history = self.load_smplx_history(
                uid, current_frame_idx_str, self.sampling_stride, min_frame_idx_int
            )
            motion_history_list.append(torch.from_numpy(skeleton_history))
        
        target_betas_tensor = torch.stack(target_betas_list, dim=0)      # (F, 10)
        target_exprs_tensor = torch.stack(target_exprs_list, dim=0)      # (F, 10)
        target_poses_tensor = torch.stack(target_pose_list, dim=0)       # (F, 55, 3)
        target_transls_tensor = torch.stack(target_transl_list, dim=0)   # (F, 3)
        ...
        return {
            ...
            'target_exprs': target_exprs_tensor,        # (F, 10)
