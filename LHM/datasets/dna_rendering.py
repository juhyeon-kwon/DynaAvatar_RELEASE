...
class DNARenderingDataset(BaseDataset):
    ...
    def inner_get_item(self, idx):
        ...
        ref_frame_idx = random.choice(tuple(face_bboxes[ref_cam_idx].keys())) # ex: '000006'

        # Load & resize REF img, mask, and REF cam_param
        ref_img_b = toc.read_image_bytes(uid, ref_cam_idx, ref_frame_idx)
        ref_mask_b = toc.read_mask_bytes(uid, ref_cam_idx, ref_frame_idx)

        # qw00n; ref_cam_param is not used now
        ...
        for _ in range(self.num_train_views):
            random_idx = random.randint(5, 59) # qw00n; near-front view # random.randint(13, 34)
        ...
        # view-dependent datas; (Frame-major list: [num_train_frames] x [num_train_views] x ...)
        ...
        # view-independent datas; (Frame-major list: [num_train_frames] x ...)
        target_betas_list = []
        target_exprs_list = []
        target_pose_list = []
        target_transl_list = []
        ...
        for i in range(self.num_train_frames): 
            # [MODIFIED] min_frame_idx와 max_frame_idx 사이에서 랜덤하게 프레임 인덱스 샘플링
        ...
        # --- 6. Final Stacking ---
        # View-dependent stack (dim=0) -> (num_train_frames, num_train_views, ...)
        ...
        # View-independent stack (dim=0) -> (num_train_frames, ...)
        ...
        return {
            ...
            'intri': intri_tensor,                  # (F, V, 4, 4)
            'extri': extri_tensor,                  # (F, V, 4, 4)
            'gt_img': gt_imgs_tensor,               # (F, V, C, H, W)
            'gt_mask': gt_masks_tensor,             # (F, V, 1, H, W)
            'is_target_face_detected': is_target_face_detected_tensor,  # (F, V)
            'target_face_bboxes_tensor': target_face_bboxes_tensor,     # (F, V, 4)
            ...
            'is_ref_face_detected': is_ref_face_detected_expanded, # (F, 1)
